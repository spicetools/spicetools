// original: https://github.com/smokris/usb-hid-usage
// Apache License 2.0
// slightly modified version

// asprintf warning fix
...
char* usb_hid_get_usage_text(uint32_t usagePage, uint32_t usage)
{
    ...
    if (usagePage == 0x01)	// Generic Desktop
    {
        ...
        text = usage < sizeof(lookup)/sizeof(char *) ? (lookup[usage] ? _strdup(lookup[usage]) : 0) : 0;
    }
    ...
    else if (usagePage == 0x20)	// Sensor
    {
        ...
        text = usageWithoutModifier < sizeof(lookup)/sizeof(char *) ? (lookup[usageWithoutModifier] ? _strdup(lookup[usageWithoutModifier]) : 0) : 0;
    }
